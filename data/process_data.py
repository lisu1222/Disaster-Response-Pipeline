...
def clean_data(df):
    ...
    row = categories.iloc[0] #extract columns names from this row
    category_colnames = [ x[0:-2] for x in row]
    categories.columns = category_colnames
    #convert category values to binary
    for column in categories:
        categories[column] = categories[column].str[-1] #only keep the last character of the string
...
def save_data(df, database_filename):
    ...
    OUTPUT
    database_filename - sqllite database
    """
